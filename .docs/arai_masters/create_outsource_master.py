"""outsource_master テーブルの作成とデータ投入。"""
...
TABLE_NAME = "outsource_master"
SHEET_NAME = "加工業者マスター"
DATA_END_COL = "C"

COLUMN_DEFS: tuple[ColumnDef, ...] = (
    ColumnDef("A", "加工業者ID", "id", "varchar", 5, primary_key=True),
    ColumnDef("B", "加工業者記号", "mark", "varchar", 5, unique=True),
    ColumnDef("C", "加工業者名", "name", "varchar", 20),
)
...
def run(refresh_mode: RefreshMode, *, skip_database_drop: bool = False) -> int:
    ...
    _, data_rows = read_sheet_rows(local_path, SHEET_NAME, DATA_END_COL, COLUMN_DEFS)
