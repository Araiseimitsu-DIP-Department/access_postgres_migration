"""ピンゲージ管理DBをAccessからPostgreSQLへ忠実に移行する対象専用スクリプト。
...
META_FILE = "ピンゲージ管理DB_meta.json"
RESULT_FILE = "migration_result_pingauge_management_db.md"
MAPPING_FILE = "migration_mapping_pingauge_management_db.md"
ERROR_LOG_FILE = "migration_error_pingauge_management_db.log"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="ピンゲージ管理DBをPostgreSQLへ移行")
...
def build_column_note(column: dict[str, Any]) -> str:
    ...
    if column["name"] == "完了フラグ":
        notes.append("Access上でVARCHAR(1)のためboolean化せず文字列で保持")
    return " / ".join(notes)
...
def infer_table_purpose(mapping: TableMapping) -> str:
    if "貸出" in mapping.access_name:
        return "ピンゲージ貸出履歴"
    if "担当者" in mapping.access_name:
        return "担当者マスタ"
    if "PG" in mapping.access_name:
        return "ピンゲージマスタ"
    return "参照テーブル"


def infer_key_columns(mapping: TableMapping) -> str:
    preferred_order = ["id", "size", "staff_id", "case_no", "machine_no", "lent_date"]
