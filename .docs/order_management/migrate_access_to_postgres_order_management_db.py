"""受注データDB.accdb を Access から PostgreSQL へ忠実に移行する対象専用スクリプト。

このファイルは .docs/order_management 専用です。
...
データ投入後、`schema_app_patches.sql` で jucyu_manager 運用向けの
索引・シーケンスを idempotent に適用する。
"""
...
META_FILE = "受注データDB_meta.json"
RESULT_FILE = "migration_result_order_management_db.md"
MAPPING_FILE = "migration_mapping_order_management_db.md"
ERROR_LOG_FILE = "migration_error_order_management_db.log"
APP_PATCHES_SQL_FILE = "schema_app_patches.sql"
DEFAULT_SCHEMA = "public"
DEFAULT_BATCH_SIZE = 1000
TARGET_OBJECT_COUNT = 16
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="受注データDB.accdb を PostgreSQL order_management へ移行")
...
def build_column_note(column: dict[str, Any]) -> str:
    ...
    if column["name"] == "金額" and column["access_type"] == "INTEGER":
        notes.append("AccessではINTEGER型のためNUMERICではなくINTEGERで保持")
    return " / ".join(notes)


def apply_app_schema_patches(connection: psycopg2.extensions.connection, schema: str) -> None:
    """jucyu_manager 運用に必要なスキーマ拡張を idempotent に適用する."""
    sql_path = TARGET_DIR / APP_PATCHES_SQL_FILE
    if not sql_path.is_file():
        raise FileNotFoundError(f"アプリ互換パッチ SQL が見つかりません: {sql_path}")
    ddl = sql_path.read_text(encoding="utf-8")
    with connection.cursor() as cursor:
        cursor.execute(f"SET search_path TO {quote_identifier(schema)}")
        cursor.execute(ddl)
    logging.info("アプリ互換パッチを適用しました: %s", sql_path.name)
...
def write_mapping(
    ...
    lines = [
        ...
        f"- 方針：受注データDB.accdb の全{TARGET_OBJECT_COUNT}テーブル・全カラムを英語スネークケースへ変換し忠実に移行",
        "",
        "## 2. 移行対象テーブル一覧",
        "",
