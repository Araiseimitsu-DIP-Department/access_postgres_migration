"""Access DB メタデータ抽出（order_performance_db）。"""
...
DB_PATH = r"C:\Users\seizo\Desktop\受注実績データ集計DB.accdb"
OUT_PATH = Path(__file__).resolve().parent / "受注実績データ集計DB_meta.json"
...
def main() -> None:
    ...
    total_rows = 0
    for table in tables:
        count = table["row_count"] or 0
        total_rows += count
        print(
            f"  [{table['table_type']}] {table['name']}: "
            f"{table['row_count']} rows, {len(table['columns'])} cols"
        )
        for col in table["columns"]:
            print(
                f"    - {col['name']}: {col['access_type']}({col['column_size']}) "
                f"nullable={col['nullable']}"
            )
    print(f"Total rows (countable): {total_rows}")
    conn.close()
