...
META_FILE = "受注実績データ集計DB_meta.json"
RESULT_FILE = "migration_result_order_performance_db.md"
MAPPING_FILE = "migration_mapping_order_performance_db.md"
ERROR_LOG_FILE = "migration_error_order_performance_db.log"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="受注実績データ集計DB.accdb を PostgreSQL order_performance_db へ移行")
...
def resolve_access_db_path(env: dict[str, str], meta: dict[str, Any]) -> Path:
    ...
    if env_path.suffix.lower() == ".laccdb":
        accdb_path = env_path.with_suffix(".accdb")
        if accdb_path.exists():
            logging.warning("ACCESS_DB_PATHが.laccdbのため、.accdb本体を使用します: %s", accdb_path)
            return accdb_path
...
def write_mapping(
    ...
    lines = [
        ...
        f"- 方針：受注実績データ集計DB.accdb の全{TARGET_OBJECT_COUNT}テーブル・全カラムを英語スネークケースへ変換し忠実に移行",
        "",
        "## 2. 移行対象テーブル一覧",
        "",
