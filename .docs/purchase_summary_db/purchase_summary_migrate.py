...
def write_mapping(metadata: dict[str, Any], result: dict[str, Any]) -> None:
    ...
    for table in metadata["tables"]:
        ...
        for index, column in enumerate(table["columns"], start=1):
            ...
            note = "Access COUNTER。主キー制約はメタデータ未検出のため要確認。" if column["access_type"] == "COUNTER" else ""
            lines.append(
    ...
    lines.extend(
        [
            ...
            "|---|---|---|",
            "| COUNTER | BIGINT GENERATED BY DEFAULT AS IDENTITY | AccessのAutoNumber相当。既存ID投入を許可。 |",
