...
- 移行日：2026-06-16 10:09:34 東京 (標準時)
- 作成者：Codex
...
| 5 | `t_購入品明細` | `purchase_detail` | TABLE | 45,582 | 45,582 | リンクテーブルではなく実テーブル |
...
|---:|---|---|---|---|---|---|
| 1 | `ID` | COUNTER | `id` | BIGINT GENERATED BY DEFAULT AS IDENTITY | 不可 | Access COUNTER。主キー制約はメタデータ未検出のため要確認。 |
