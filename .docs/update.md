...
- `.docs/pingauge_management_db/setup_id_identity_pingauge_management_db.py` を追加し、`pin_gauge_lending.id` に `GENERATED BY DEFAULT AS IDENTITY` を設定。既存の Access COUNTER 値は保持し、新規 INSERT 時のみ自動採番する。
