"""協力会社委託加工処理品DB.accdb を Access から PostgreSQL へ忠実に移行する対象専用スクリプト。

このファイルは .docs/subcon_manager 専用です。
...
META_FILE = "協力会社委託加工処理品DB_meta.json"
RESULT_FILE = "migration_result_subcon_manager_db.md"
MAPPING_FILE = "migration_mapping_subcon_manager_db.md"
ERROR_LOG_FILE = "migration_error_subcon_manager_db.log"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="協力会社委託加工処理品DB.accdb を PostgreSQL subcon_manager へ移行")
...
def write_mapping(
    ...
    lines = [
        ...
        f"- 方針：協力会社委託加工処理品DB.accdb の全{TARGET_OBJECT_COUNT}テーブル・全カラムを英語スネークケースへ変換し忠実に移行",
        "",
        "## 2. 移行対象テーブル一覧",
        "",
