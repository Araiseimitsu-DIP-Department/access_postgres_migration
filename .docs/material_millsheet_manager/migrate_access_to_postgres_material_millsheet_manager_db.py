...
データ投入後、`schema_app_patches.sql` で incoming_material_inspection 運用向けの
主キー・索引・BOOLEAN デフォルトを自動適用します。
"""
...
def apply_app_schema_patches(connection: psycopg2.extensions.connection, schema: str) -> None:
    """incoming_material_inspection 運用に必要なスキーマ拡張を idempotent に適用する。"""
