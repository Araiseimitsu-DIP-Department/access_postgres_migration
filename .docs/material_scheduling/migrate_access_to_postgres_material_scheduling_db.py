"""セット予定材料管理DBをAccessからPostgreSQLへ忠実に移行する対象専用スクリプト。

このファイルは .docs/material_scheduling 専用です。
...
データ投入後、`schema_app_patches.sql` で material_management_system 運用向けの
列拡張・シーケンス・索引を自動適用します。
"""
...
META_FILE = "セット予定材料管理DB_meta.json"
RESULT_FILE = "migration_result_material_scheduling_db.md"
MAPPING_FILE = "migration_mapping_material_scheduling_db.md"
ERROR_LOG_FILE = "migration_error_material_scheduling_db.log"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="セット予定材料管理DBをPostgreSQLへ移行")
...
def build_column_note(column: dict[str, Any]) -> str:
    ...
    if column["name"] in {"完了フラグ", "表示フラグ", "手配済フラグ", "注文書印刷済フラグ", "材料入荷完了フラグ", "主担当フラグ", "使用フラグ", "指示書有無"}:
        notes.append("Access上でVARCHAR(1)相当のため文字列で保持")
    return " / ".join(notes)


def apply_app_schema_patches(connection: psycopg2.extensions.connection, schema: str) -> None:
    """material_management_system 運用に必要なスキーマ拡張を idempotent に適用する。"""
